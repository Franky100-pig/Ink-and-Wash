...
;(function () {
  ...
  SandSim.prototype.buildLUT = function () {
    var LUT = new Uint32Array(TYPE_COUNT * 2 * 256)
    for (var t = 0; t < TYPE_COUNT; t++) {
      for (var s = 0; s < 2; s++) {
        for (var m = 0; m < 256; m++) {
          ...
          LUT[(t << 9) | (s << 8) | m] = packRGBA(clampByte(r), clampByte(g), clampByte(b), 255)
        }
      }
    }
    this.LUT = LUT
  ...
  SandSim.prototype.resize = function () {
    ...
    var oldType = this.type, oldMeta = this.meta, oldStain = this.stain
  ...
  SandSim.prototype.render = function (/* time */) {
    ...
    var LUT = this.LUT
    var type = this.type, meta = this.meta, stain = this.stain
    for (var i = 0; i < N; i++) {
      buf[i] = LUT[(type[i] << 9) | (stain[i] << 8) | meta[i]]
    }
  ...
  SandSim.prototype.clear = function () {
    this.type.fill(0); this.meta.fill(0); this.stain.fill(0)
    this.counts.fill(0)
  ...
  SandSim.prototype.dispose = function () {
    this.type = this.meta = this.stain = this.moved = this.counts = null
    this.buf32 = null
  }
