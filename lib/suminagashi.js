...
// 适配说明：本文件改编自 https://github.com/fisheryv/healing（MIT，Copyright (c) 2026 Fisher）。
// 原文件使用 `import * as THREE from 'three'`。为了让页面零构建、完全离线（直接双击 index.html 即可运行），
// 这里改为使用 lib/three.min.js（UMD）提供的全局 THREE。另新增了 clear() 方法（原引擎没有清屏能力），其余逻辑不变。
...
const INKS = {
  ...
  white: new THREE.Color('#ffffff'),  // 白：减淡/留白（橡皮），见 dropInk/strokeInk 的负吸光度处理
}
...
class Suminagashi {
  constructor(canvas, opts = {}) {
    this.config = {
      ...
      DYE_DISSIPATION: 0.0,    // 关掉染料消散：墨一旦落下就永久留得住（疗愈场景要“留得住”，不留白漂回纸色）；想擦用白色墨。
      CURL: 14,
      SPLAT_RADIUS: 0.0026,
      SPLAT_FORCE: 5200,
      SPLAT_VELOCITY: 1.0,  // 速度注入乘子：精细模式（fine art）调低后笔触更“停”，扩散明显减少
      ...opts,
    }

    this.inkGain = 1.0  // 浓度倍率：由 UI “浓度”滑块控制（只影响墨色深浅，不影响扩散）
  ...
  _initShaders() {
    ...
    this.displayMat = prog(/* glsl */`
      ...
      uniform vec3 uPaper;
      uniform float uTime;
      uniform float uGain;   // 浓度：渲染端统一加深整张纸的墨层（含 AI 与已落的墨）
      ...
      void main(){
        ...
        vec3 A = texture2D(uDye, vUv).rgb * uGain;
  ...
  dropInk(x, y, color, strength) {
    const abs = this._inkAbsorption(color, strength)
    this.splatDye(x, y, abs, 1.0)
    if (color !== INKS.white) {
      const angle = Math.random() * Math.PI * 2
      const speed = (60 + Math.random() * 80) * (this.config.SPLAT_VELOCITY || 1)
  ...
  strokeInk(x, y, color, strength, dirX, dirY, radiusMul = 0.8) {
    ...
    // 沿笔触方向添加速度（让墨水沿笔迹流动，而非四散溅射；白色也沿笔触刷出“留白”）
    const v = (30 + strength * 50) * (this.config.SPLAT_VELOCITY || 1)
    this.splatVelocity(x, y, dirX * v, dirY * v, 1.0)
  }

  /** 计算落墨的吸光度矢量；白色墨返回负吸光度（减淡/留白） */
  _inkAbsorption(color, strength) {
    if (color === INKS.white) {
      const a = strength * 0.22
      return new THREE.Vector3(a, a, a).multiplyScalar(-1)
    }
    return inkAbsorption(color, strength * 0.22)
  }
  ...
  render(time = 0) {
    ...
    this.displayMat.uniforms.uGain.value = this.inkGain
  ...
  /** 清空画布与速度场（引擎继续运行，墨会从空白重新洇开） */
  clear() {
    const zeroMat = new THREE.ShaderMaterial({
      vertexShader: VERT,
      fragmentShader: 'precision highp float; varying vec2 vUv; void main(){ gl_FragColor = vec4(0.0); }',
      depthTest: false,
      depthWrite: false,
    })
    this.quad.material = zeroMat
    for (const rt of [this.dye.read, this.dye.write, this.velocity.read, this.velocity.write]) {
...
// 暴露为全局，供 app.js（经典脚本）使用；保持 MIT 署名。
if (typeof window !== 'undefined') {
  window.Suminagashi = Suminagashi
  window.INKS = INKS
  window.INK_KEYS = INK_KEYS
}
